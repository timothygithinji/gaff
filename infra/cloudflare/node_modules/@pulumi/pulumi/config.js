...
class Config {
    ...
    requireImpl(key, secret, opts, use, insteadOf) {
    ...
    requireBooleanImpl(key, secret, use, insteadOf) {
    ...
    requireNumberImpl(key, secret, opts, use, insteadOf) {
    ...
    requireObjectImpl(key, secret, use, insteadOf) {
     ...
     */
    requireObject(key) {
        return this.requireObjectImpl(key, false, this.requireSecretObject, this.requireObject);
    }
    /**
    ...
    requireSecretObject(key) {
        return makeSecret(this.requireObjectImpl(key, true));
    }
    /**
...
class ConfigMissingError extends errors_1.RunError {
    constructor(key, secret) {
        super(`Missing required configuration variable '${key}'\n` +
            `\tplease set a value using the command \`pulumi config set${secret ? " --secret " : " "}${key} <value>\``);
        this.key = key;
        this.secret = secret;
    }
}
