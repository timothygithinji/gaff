...
function throwOrPrintModuleLoadError(program, error) {
    ...
    let packageObject;
    try {
        packageObject = manifest_1.readPackageManifest(projectRoot).data;
    }
    catch {
    ...
    const runProgram = async () => {
        ...
        try {
            ...
            if (components.length > 0) {
                ...
                const { data: packageJSON, path: packageJSONPath } = manifest_1.readPackageManifest(absDir);
                if (!packageJSON?.name) {
                ...
                if (!/^[a-zA-Z]/.test(providerName)) {
                    throw new Error(`Invalid provider name '${providerName}' in ${path.basename(packageJSONPath)}. ` +
