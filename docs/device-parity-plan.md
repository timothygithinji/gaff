...
2. **Shared primitives + shapers:** build the primitives both screens need — `SectionCard` (done), `PriceBlock`, `FeaturePills`+`toPills`, `PortalList`+`toPortalRows`, `StatRow`+`toStatCells`, `EmptyState` — each to the converged spec with its pure shaper + `renderToStaticMarkup` contract test.
