...
(auto-mutual with self), couple (Tim + Partner), N (every flatmate).
...
| Shortlist tabs | "Saved" only | `Mutual N · Yours N · Partner's N` | `Mutual N · Yours N · <per member>` |
| "BOTH KEPT" badge | drop | "BOTH KEPT" | "ALL KEPT" / "4 of 4 KEPT" with avatar stack |
| Listing detail CTA | "Keep" | "Keep · waiting on Partner" | "Keep · waiting on Partner + 2 others" |
...
- Add Partner's email to the Pulumi CF Access policy
