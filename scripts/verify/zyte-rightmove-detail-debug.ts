...
const wrapper = JSON.parse(extractWindowPageModelString(res.html)) as {
  data: string;
};
...
try {
  ...
  console.log(
    `✓ Direct JSON.parse works. Type: ${Array.isArray(direct) ? "array" : typeof direct}`
  );
  if (Array.isArray(direct)) {
    ...
    if (typeof pdRef === "number") {
      ...
      console.log(
        `  Resolved propertyData keys (${Object.keys(pd).length}): ${Object.keys(pd).slice(0, 30).join(", ")}`
      );
      // Sample some integer values to see if they're also refs
      const sampleKey = "bedrooms";
      if (sampleKey in pd) {
        console.log(
          `  pd.${sampleKey} = ${pd[sampleKey]} (type ${typeof pd[sampleKey]})`
        );
      }
      const fields = [
        "id",
...
console.log(
  `Reflect.ownKeys: ${own.length} → ${own.map(String).slice(0, 20).join(", ")}`
);
const fpAny = fp as Record<string, unknown>;
if (fpAny.propertyData) {
  const pd = fpAny.propertyData;
  console.log(
    `flatted root.propertyData type: ${typeof pd}, value: ${typeof pd === "object" ? "(object)" : pd}`
  );
  if (pd && typeof pd === "object") {
    const pdKeys = Reflect.ownKeys(pd);
    console.log(
      `pd Reflect.ownKeys: ${pdKeys.length} → ${pdKeys.map(String).slice(0, 20).join(", ")}`
    );
  }
}
