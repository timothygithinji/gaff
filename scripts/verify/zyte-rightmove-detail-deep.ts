...
for (const m of primaryPriceMatches.slice(0, 4)) {
  ...
  const slice = html.slice(start, idx + 200);
  console.log(`\nContext around primaryPrice @ ${idx}:`);
...
for (let i = 0; i < lds.length; i++) {
  try {
    ...
    console.log(
      `  [${i}] @type: ${(parsed as { "@type"?: string })["@type"] ?? "(none)"}`
    );
    console.log(
      `  [${i}] keys: ${Object.keys(parsed).slice(0, 15).join(", ")}`
    );
  } catch (err) {
...
const winStateMatches = [
  ...html.matchAll(/window\.(__[A-Z_]+|[a-zA-Z_$]+)\s*=\s*(\{[\s\S]*?\});/g),
];
...
for (const m of bedroomMentions) {
  console.log(`  "${m[0]}" at idx ${m.index}`);
}

const priceMatches = [
  ...html.matchAll(/£[\d,]+(?:\s*pcm|\s*pw|\s*per\s*month)?/gi),
].slice(0, 4);
...
const apiUrls = [
  ...new Set(
    [...html.matchAll(/(?:fetch|axios)\(["'](\/[^"']+)["']/g)].map((m) => m[1])
  ),
];
console.log(`\nDetected API fetch URLs (${apiUrls.length}):`);
for (const u of apiUrls.slice(0, 10)) {
  console.log(`  ${u}`);
}

const apiUrls2 = [
  ...new Set(
    [...html.matchAll(/["'](https?:\/\/[^"']*?\/api\/[^"']+)["']/g)].map(
      (m) => m[1]
    )
  ),
];
console.log(`Detected /api/ URLs anywhere (${apiUrls2.length}):`);
for (const u of apiUrls2.slice(0, 10)) {
  console.log(`  ${u}`);
}
