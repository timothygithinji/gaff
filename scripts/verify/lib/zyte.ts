...
export async function zyteFetch(
  ...
  if (!res.ok) {
    ...
    throw new Error(`Zyte ${res.status} ${res.statusText}: ${body.slice(0, 400)}`);
  }
  ...
  let html = "";
  if (data.browserHtml) html = data.browserHtml;
  else if (data.httpResponseBody)
    html = new TextDecoder().decode(Uint8Array.from(atob(data.httpResponseBody), (c) => c.charCodeAt(0)));

  const headers: Record<string, string> = {};
  for (const h of data.httpResponseHeaders ?? []) headers[h.name.toLowerCase()] = h.value;
  ...
  const costEstimateUsd = Number.parseFloat(res.headers.get("zyte-request-cost") ?? "0") || 0;

  console.log(`  → fetched in ${ms}ms · status ${data.statusCode ?? "?"} · cost $${costEstimateUsd.toFixed(5)}`);
