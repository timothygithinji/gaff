...
const pd = findByKey(flight, "propertyData") as {
...
if (pd?.propertyData) {
  ...
  console.log(
    `  Keys (${Object.keys(d).length}): ${Object.keys(d).slice(0, 25).join(", ")}`
  );
} else {
  ...
  const candidate = findInFlight(
    ...
    (v) =>
      ...
      typeof v === "object" &&
      !Array.isArray(v) &&
      "bedrooms" in (v as object) &&
      "prices" in (v as object)
  );
  if (candidate) {
    const c = candidate as Record<string, unknown>;
    console.log(
      `✓ Found via bedrooms+prices probe. Keys: ${Object.keys(c).slice(0, 25).join(", ")}`
    );
  } else {
    ...
    if (c2) {
      ...
      console.log(
        `✓ Found via bedrooms+id probe. Keys: ${Object.keys(c).slice(0, 30).join(", ")}`
      );
    } else {
