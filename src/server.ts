...
/**
 * Serve a listing photo from R2, fronted by the colo edge cache.
 *
 * A Worker's `new Response()` is NOT stored in Cloudflare's CDN cache
 * automatically, so without this every photo view — even the same image
 * seen a hundred times — re-invokes the Worker AND issues a fresh R2 `GET`
 * subrequest. The cache key is a bare GET on the URL (the `?w=` width is
 * part of it; cookies/headers are dropped) so it never varies per user —
 * these objects are public + immutable.
 */
async function servePhoto(url: URL): Promise<Response> {
  // `caches.default` is the Workers runtime cache. The DOM lib's
  // `CacheStorage` type (also in scope here) doesn't declare `.default`, so
  // reach it through a cast; the DOM `Cache` shape covers match/put.
  ...
  // Stamp immutable caching + persist to the colo cache off the response
  // path (`waitUntil`), so the write never adds latency to this request.
  const serve = (object: R2ObjectBody): Response => {
  ...
  // inside Cloudflare's free Image-Transformations tier (it bills per unique
  // transform and capped us out in one browse). A bucket at/above the source
  // width was never generated, so the lookup misses and we fall through to
  // the full-size original below — the old "scale-down" promise.
  ...
  // Object isn't in the bound bucket. This is the normal case in local dev:
  // `cache-photos` uploads to the real remote bucket over the S3 API, so the
  // miniflare bucket `vite dev` binds stays empty — yet the DB (especially
  // under `dev:prod`) still hands back `clusters/…` keys. Fall back to the
  // portal URL stored alongside the key so images render instead of 404ing.
  // In prod the object normally exists so we never reach here, but an evicted
  // object now degrades gracefully too. The key is
  // `clusters/{cluster}/listings/{listingId}/{file}`; pull the listingId out
  // so the lookup rides the listing_id index.
  ...
  if (listingId) {
    const [row] = await getDb()
      ...
      .from(listingPhotos)
      .where(
        and(eq(listingPhotos.listingId, listingId), eq(listingPhotos.r2Key, key))
      )
      .limit(1);
    if (row?.url) {
