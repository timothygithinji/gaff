...
export async function findCoveringOutcodes(
  ...
  // postcodes.io's OpenAPI declares `latitude`/`longitude` with a
  // multi-type schema (`["number", "null"]`) — the generator widens
  // both to `unknown`, so we coerce defensively here. The runtime
  // values are always finite numbers for outcodes that actually have a
  // centroid (most do).
  type CentroidPick = { outcode: string; latitude: unknown; longitude: unknown };
  const inside: Array<{ outcode: string; lat: number; lng: number }> = [];
  for (const o of data.result as CentroidPick[]) {
    ...
    inside.push({ outcode: o.outcode, lat, lng });
  }

  // Sort closest-to-centre first so the UI chip list reads naturally.
  const sorted = inside
    .map((o) => ({
      outcode: o.outcode.toUpperCase(),
      distance: distanceMetres(center, { lat: o.lat, lng: o.lng }),
    }))
