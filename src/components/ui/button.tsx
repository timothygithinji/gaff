...
type ButtonProps = ButtonPrimitive.Props &
  VariantProps<typeof buttonVariants> & {
    /**
     * Show a spinning loading indicator inline with (or in place of)
     * the button's children. Also sets `aria-busy` and `disabled` so
     * the button can't be re-fired while a mutation is in flight. When
     * `loadingText` is set, the children are replaced with that label
     * next to the spinner; otherwise the spinner sits before the
     * existing children unchanged.
     */
    loading?: boolean;
    /** Optional text to show next to the spinner while loading. */
