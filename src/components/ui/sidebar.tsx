...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // biome-ignore lint/nursery/noDocumentCookie: shadcn persists sidebar state via cookie; safe small string, no PII.
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: shadcn original — setters from useState are stable.
  const toggleSidebar = React.useCallback(() => {
    return isMobile ? setOpenMobile((open) => !open) : setOpen((open) => !open);
  }, [isMobile, setOpen, setOpenMobile]);

  // Cmd/Ctrl + B toggles the sidebar. `Mod` resolves to Meta on macOS,
  // Control elsewhere — matches the original shadcn behavior.
  //
  // NOTE: this is a deliberate divergence from shadcn upstream. The
  // generator ships a raw `window.addEventListener("keydown", …)` here;
  // we've replaced it with `useHotkey` so the shortcut shows up in the
  // app-wide help dialog and the TanStack Hotkeys devtools. If you re-run
  // `bunx shadcn add sidebar`, you'll need to re-apply this swap.
  useHotkey("Mod+B", () => toggleSidebar(), {
    meta: { category: "App", description: "Toggle sidebar" },
  });
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: shadcn original — every member is already listed.
    ...
    >
      ...
      >
        <div
          ...
          className="flex size-full flex-col border-border border-r bg-sidebar group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:shadow-sm group-data-[variant=floating]:ring-1 group-data-[variant=floating]:ring-sidebar-border"
        >
          {children}
        </div>
      </div>
...
function SidebarMenuBadge({
  ...
  return (
    <div
      ...
      className={cn(
        "pointer-events-none absolute right-1 flex h-5 min-w-5 select-none items-center justify-center rounded-md border border-sidebar-border bg-sidebar-accent px-1 font-medium text-sidebar-accent-foreground text-xs tabular-nums peer-hover/menu-button:text-sidebar-accent-foreground group-data-[collapsible=icon]:hidden peer-data-[size=default]/menu-button:top-1.5 peer-data-[size=lg]/menu-button:top-2.5 peer-data-[size=sm]/menu-button:top-1 peer-data-active/menu-button:text-sidebar-accent-foreground",
        className
      )}
      {...props}
    />
